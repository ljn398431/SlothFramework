...
public class AddressableImporter : AssetPostprocessor
{
    ...
    static void ProcessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
    {
        ...
        string assetPath)
    {
        ...
        if (rule.groupTemplate != null && (newGroup || rule.groupTemplateApplicationMode == GroupTemplateApplicationMode.AlwaysOverwriteGroupSettings))
        {
            ...
            group.GetSchema<ContentUpdateGroupSchema>().StaticContent = rule.isStatic;
            else
            {
                group.AddSchema<ContentUpdateGroupSchema>().StaticContent = rule.isStatic;
            }
        }
