...
local function build_data(data, path, self_callback, root)
	...
	local methods = {
		...
		insert = function(_, value, index)
			local k = index and index or #_data + 1
			local cbs = {}
			table.insert(callbacks, k, cbs)
			table.insert(_data, k, type(value) == "table" and build_data(value, append_key(path, k), cbs, root) or value)
			data.__notify_self()
		end,
