...
---@class Game.Actions.TextAndImageAction
local M = class(AIActionBase)

function M:ctor(serverData)
    self.serverData = serverData
    self.duration = tonumber((serverData.duration or 0) / 1000)
    warn("TextAndImageAction:ctor", table.dump_r(serverData))
end

function M:GetActionName()
    return "TextAndImageAction"
end

function M:OnEnter()
    M.super.OnEnter(self)
    warn("TextAndImageAction:OnEnter", table.dump_r(self.serverData))
    SM.GetGlobalEventService().GetGlobalDispatcher():DispatchEvent(EventType.QUESTION_ANSWER,
            { content = self.serverData.content,
              imageUrl = self.serverData.imageUrl,
              seq01 = self.serverData.seq01 })
end
